...
function install_ss_panel_mod_v3(){
	...
	cp config/.config.php.example config/.config
	...
	wget -N -P /home/wwwroot/public/ https://raw.githubusercontent.com/echo-marisn/ssrv3-one-click-script/master/favicon.ico
	chattr -i .user.ini
	mv .user.ini public
	chown -R root:root *
	chmod -R 777 *
...
function install_node(){
	...
	read -p "请输入你的对接域名或IP(请加上http:// 回车默认为本机): " Userdomain
	read -p "请输入muKey(回车默认mupass):" Usermukey
	read -p "请输入你的节点编号(回车默认为3):  " UserNODE_ID
...
EOF
    ...
    sed -i 's/$System_Config['db_password'] = '${Default_password}'/$System_Config['db_password'] = '${Change_password}'/g' /home/wwwroot/default/config/.config.php
